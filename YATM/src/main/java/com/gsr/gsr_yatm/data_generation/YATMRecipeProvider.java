...
public class YATMRecipeProvider extends RecipeProvider
{
	...
	{	
		...
		this.addBioreactionRecipes(writer);
		...
		this.addInjectingRecipes(writer);
	...
	private void addCraftingRecipes(@NotNull Consumer<FinishedRecipe> writer) 
	{
		...
		ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, Items.DIRT, 8).requires(Ingredient.of(YATMItems.BIO_BUCKET.get()), 1).requires(Ingredient.of(YATMItemTags.SOIL_MINERALS_KEY), 8).unlockedBy("criteria", inventoryTrigger(ItemPredicate.Builder.item().of(YATMItems.BIO_BUCKET.get()).build())).save(writer, YetAnotherTechMod.MODID + ":dirt_from_minerals_and_biofluid_shapeless_crafting");
		// TO BECOME GRAFTING \\
...
//		this.addPartExchange(writer, YATMItemTags.SAWS_KEY, new ItemStack(YATMItems.DIAMOND_SAW_BLADE.get()), new ItemStack(YATMItems.STEEL_SAW_DIAMOND.get()), "saw_blade_adjustment", YetAnotherTechMod.MODID + ":steel_saw_diamond_from_part_exchange");
	...
	{
		this.addSmelting(writer, new ItemLike[] {YATMItems.LATEX_BUCKET.get()}, YATMItems.RUBBER_BLOCK_ITEM.get(), .3f, 60, YetAnotherTechMod.MODID + ":rubber_block_from_latex_smelting");
		...
		this.addSmelting(writer, new ItemLike[] {YATMItems.BIO_BUCKET.get()}, Items.BONE_MEAL, 1.0f, 100, YetAnotherTechMod.MODID + ":bonemeal_from_bio_bucket_smelting");
	...
	private void addBioreactionRecipes(@NotNull Consumer<FinishedRecipe> writer) 
	{
		this.addChorusBioreactions(writer);
	...
	private void addChorusBioreactions(Consumer<FinishedRecipe> writer) 
	{
	...
	private void addRootedSoilReversions(@NotNull Consumer<FinishedRecipe> writer) 
	{
	...
	private void addGrinderBrickCrackings(@NotNull Consumer<FinishedRecipe> writer) 
	{
	...
	private void addGrinderStoneTowardsCobblestone(@NotNull Consumer<FinishedRecipe> writer) 
	{
	...
	private void addGrindToSand(@NotNull Consumer<FinishedRecipe> writer) 
	{
	...
	private void addGrindPurpur(@NotNull Consumer<FinishedRecipe> writer) 
	{
	...
	private void addInjectingRecipes(@NotNull Consumer<FinishedRecipe> writer)
	{
		// TODO, should the biofluid be a fluid tag?
		new InjectingRecipeBuilder().input(new FluidStackIngredient(new FluidStack(YATMFluids.BIO.get(), 80))).substrate(new ItemTagIngredient(YATMItemTags.SOIL_MINERALS_KEY)).result(new ItemStack(Items.DIRT)).currentPerTick(18).timeInTicks(60).unlockedBy("has_ingredient", inventoryTrigger(ItemPredicate.Builder.item().of(YATMItemTags.SOIL_MINERALS_KEY).build())).save(writer, new ResourceLocation(YetAnotherTechMod.MODID, "dirt_from_biofluid_mineral_injecting"));
